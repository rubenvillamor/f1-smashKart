...
class Game {
  ...
  vehiculoAparece = () => {
    ...
    if (
      (gameInProgress && this.enemyCars.length === 0) ||
      this.frames % 60 === 0
    ) {
      if (this.enemyCars.length < 18 && !verstappenAdded) {
    ...
    if (
      verstappenAdded &&
      alonso.x < this.enemyCars.find((car) => car.name === "Verstappen").x
    ) {
